...
const pos = {
    ...
    'topRight': { x: 36.1, y: 73.5, z: 0.57 },
    'topLeft': { x: -32.5, y: 74.4, z: 0.816 },
...
function init() {
    ...
    Promise.all([
        ...
        loadModel('../assets/ring7.glb').then(result => {
        ...
        changeText('WESTERN', 'topleft');
        changeText('HIGHSCHOOL', 'topright');
        changeText('2018', 'left');
        changeText('TIFFA', 'right');
...
function changeText(text, side) {
    ...
    switch (side) {
        case 'topright':
            removeChars('topright');
            for (var i = 0; i < L; ++i) {
                temp = getMesh(text.charCodeAt(i), 'cambria', 'bold');
                let m = temp.clone();
                var a = data['topright_' + L + '_' + (L - i)];
        ...
        case 'topleft':
            ...
            for (var i = 0; i < L; ++i) {
                temp = getMesh(text.charCodeAt(i), 'cambria', 'bold');
                let m = temp.clone();
                var a = data['topleft_' + L + '_' + (L - i)];
                m.position.set(a.position[0], a.position[1], a.position[2]);
                rotate(m, a.rotation);
                m.scale.set(-a.scale[0], a.scale[1], -a.scale[2]);
                m.visible = true;
                m.material = ring.body[0].material;
                charPos.topleft.push(m);
                scene.add(m);
            }
            break;
        case 'right':
        ...
        case 'inside':
            ...
            for (var i = 0; i < L; ++i) {
                ...
                if (L % 2) {
                    ...
                    m.position.set(a.position[0], a.position[1], a.position[2]);
                    rotate(m, a.rotation);
                    m.scale.set(a.scale[0], 0.95, a.scale[2]);
                    m.visible = true;
...
//CHANGE TOP_RIGHT TEXT 
document.getElementById('top_right').onfocus = function () {
    console.log('awef')
    moveCamera(pos.topRight);
}
document.getElementById('top_right').onkeyup = function () {
    moveCamera(pos.topRight);
    var str = this.value.replace(/[^ -~]+/g, "");
    str = str.toUpperCase();
    str = str.replace(/[`{}_\[\]\\|^]/g, '');
    this.value = str;
    changeText(str, 'topright');
}

//CHANGE TOP_LEFT TEXT 
document.getElementById('top_left').onfocus = function () {
    moveCamera(pos.topLeft);
}
document.getElementById('top_left').onkeyup = function () {
    moveCamera(pos.topLeft);
    var str = this.value.replace(/[^ -~]+/g, "");
    str = str.toUpperCase();
    str = str.replace(/[`{}_\[\]\\|^]/g, '');
    this.value = str;
    changeText(str, 'topleft');
}
