...
function init() {
  ...
  Promise.all([
    ...
    loadModel('../assets/ring4.glb').then((result) => {
    ...
    changeText('NANCY', 'right')
    changeText('2019', 'left')
  ...
  switch (side) {
    ...
    case 'right':
      ...
      for (var i = 0; i < L; ++i) {
        ...
        m.position.set(a.position[0], a.position[1], a.position[2])
        rotate(m, a.rotation)
        m.scale.set(-a.scale[0], 0.72, -a.scale[2])
        m.visible = true
    ...
    case 'left':
      ...
      for (var i = 0; i < L; ++i) {
        ...
        m.position.set(a.position[0], a.position[1], a.position[2])
        rotate(m, a.rotation)
        m.scale.set(a.scale[0], 0.7, a.scale[2])
        m.visible = true
        m.material = new THREE.MeshBasicMaterial({ color: 0x000000 })
        charPos.left.push(m)
        scene.add(m)
