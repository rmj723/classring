...
async function init() {
  const settings = await initScene(container, 55, {
    ring,
    graphs,
    chars,
    fonts,
  ...
  switch (side) {
    case "neck":
      ...
      for (var i = 0; i < text.length; ++i) {
        ...
        m.scale.set(1.15, 0.3, 1.2);
        if (text.length > 20) m.scale.x = 1;
        // tc.showCurve();
        scene.add(m);
      }
    ...
    case "left":
      ...
      for (var i = 0; i < text.length; ++i) {
        ...
        sideCurve.generatePose(m, i, -0.7);
        m.scale.set(0.8, 0.6, 1);
        m.scale.x = 1.45 - 0.1 * text.length;

        m.geometry.translate(0, 0.8, 0);
