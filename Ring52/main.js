...
async function init() {
  ...
  const ringGLTF = await loader.loadAsync("../assets/ring52.glb");
...
function changeGraph(index, side) {
  ...
  const x = 10.5;
  ring[side].position.set(side === "left" ? -x : x, 3.6, 0.07);
  ...
  switch (side) {
    ...
    case "right":
      ...
      for (var i = 0; i < text.length; ++i) {
        ...
        m.scale.x = 1.7 - 0.11 * text.length;
    ...
    case "left":
      ...
      for (var i = 0; i < text.length; ++i) {
        ...
        let m = temp.clone();
        leftCurve.generatePose(m, i, 0.95);
        m.scale.set(0.8, 0.6, 1.28);
        m.scale.x = 1.7 - 0.11 * text.length;
