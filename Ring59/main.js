...
async function init() {
  const settings = await initScene(container, 59, {
    ring,
    graphs,
    chars,
    fonts,
...
function changeText(t, side) {
  ...
  switch (side) {
    ...
    case "top2":
      ...
      const topCurve = new TextCurve(
        side === "top1" ? top1CurveData : top2CurveData,
        text.length
      );
      ...
      for (var i = 0; i < text.length; ++i) {
        ...
        if (side === "top1") {
          topCurve.generatePose(m, i, 0.45);
          m.scale.set(1.4, 0.15, 0.8);
          m.scale.x = 1.6 - 0.08 * text.length;
        } else {
          topCurve.generatePose(m, i, -0.45);
          m.scale.set(1.4, 0.3, 0.8);
          m.scale.x = 1.6 - 0.08 * text.length;
        }

        // topCurve.showCurve(scene);
        scene.add(m);
      }
      break;
...
//CHANGE TOP GRAPH
el("top_graph").onclick = () => moveCamera(pos.topText2);
