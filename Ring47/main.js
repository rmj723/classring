...
const p = {
  inside: { fontSize: 30 * r, s: 0, e: 450 * r, left: 0, top: 30 * r },
  left: { fontSize: 34 * r, s: 11 * r, e: 240 * r, top: 93 * r },
  right: { fontSize: 34 * r, s: 11 * r, e: 250 * r, top: 160 * r },
};
var delta = 300;
...
async function init() {
  ...
  const ringGLTF = await loader.loadAsync("../assets/ring47.glb");
...
function changeRing({
  ...
  drawContent(content);
  changeText(leftTopText, "left_top");
  changeText(rightTopText, "right_top");
...
function changeText(text, side) {
  ...
  switch (side) {
    case "neck":
      ...
      break;
    case "right_top":
      removeChars("right_top");
      for (var i = 0; i < L; ++i) {
        temp = getMesh(text.charCodeAt(i), "arial", "bold");
        let m = temp.clone();
        let a = data["right_top_" + (i + 1)];
        m.position.set(a.position[0], a.position[1], a.position[2]);
        rotate(m, a.rotation);
      ...
      break;
    case "left_top":
      removeChars("left_top");
      for (var i = 0; i < L; ++i) {
        temp = getMesh(text.charCodeAt(i), "arial", "bold");
        let m = temp.clone();
        let a = data["left_top_" + (i + 1)];
        m.position.set(a.position[0], a.position[1], a.position[2]);
        rotate(m, a.rotation);
        m.scale.set(a.scale[0], 1.2, a.scale[2]);
        m.visible = true;
...
el("left_top_text").onkeyup = () => {
  ...
  changeText(el("left_top_text").value, "left_top");
};
...
el("right_top_text").onkeyup = () => {
  ...
  changeText(el("right_top_text").value, "right_top");
};
