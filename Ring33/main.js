...
var charPos = { neck: [], right: [], left: [], inside: [], top: [] };
var ring = {
  body: null,
  material: null,
  textures: {},
...
const pos = {
  ...
  topText: { x: -12.96, y: 67.8, z: 25.5 },
};
var ctx,
  overflow = {};
const p = { inside: { fontSize: 33, s: -20, e: 480, left: 0, top: 55 } };
var delta = 300;
...
async function init() {
  ...
  const ringGLTF = await new GLTFLoader().loadAsync("../assets/ring33.glb");
  ...
  switch (side) {
    ...
    case "neck":
      ...
      for (var i = 0; i < L; ++i) {
        ...
        m.position.set(a.position[0], a.position[1], a.position[2]);
        rotate(m, a.rotation);
        m.scale.set(a.scale[0] + 0.4, a.scale[1] - 0.2, a.scale[2]);
        m.visible = true;
        m.material = ring.material;
        charPos.neck.push(m);
        scene.add(m);
