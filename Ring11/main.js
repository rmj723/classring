...
const pos = {
    'left': { x: -79.64, y: 9.57, z: 14.8 },
    'right': { x: 75.17, y: 17.47, z: -21.11 },
    'topCore': { x: -18.2, y: 76, z: 24.8 },
    'neckText': { x: -13.58, y: 62.62, z: -46.44 },
    'rightText': { x: 67.144, y: 38.98, z: 2.04 },
    'leftText': { x: -74.18, y: 28.57, z: -4.87 },
...
function init() {
    ...
    Promise.all([
        ...
        loadModel('../assets/ring11.glb').then(result => {
...
function changeText(text, side) {
    ...
    switch (side) {
        case 'top':
            removeChars('top');
            temp = getMesh(text.charCodeAt(0), 'cambria', 'bold');
            let m = temp.clone();
            var a = data['top_1_1'];
        ...
        case 'inside':
            ...
            for (var i = 0; i < L; ++i) {
                ...
                if (L % 2) {
                    ...
                    m.position.set(a.position[0], a.position[1], a.position[2]);
                    rotate(m, a.rotation);
                    m.scale.set(a.scale[0], 1, a.scale[2]);
                    m.visible = true;
