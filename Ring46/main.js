...
var ctx,
  overflow = {};
const p = { inside: { fontSize: 34, s: -20, e: 480, left: 0, top: 45 } };
var delta = 300;
...
async function init() {
  ...
  const ringGLTF = await loader.loadAsync("../assets/ring46.glb");
  ...
  switch (side) {
    case "top1":
      ...
      for (var i = 0; i < L; ++i) {
        ...
        rotate(m, a.rotation);
        m.scale.set(a.scale[0] + 0.3, a.scale[1] - 0.2, a.scale[2]);
        m.visible = true;
