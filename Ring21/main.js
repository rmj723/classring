...
var ring = {body: null, material: null, textures: {}, core: null, graph: null};
const pos = {
...
async function init() {
  ...
  const ringGLTF = await new GLTFLoader().loadAsync('../assets/ring21.glb');
...
function changeRing({ringColor, graph, insideText, topText}) {
...
function changeGraph(n) {
  ...
  ring.graph.material = ring.material;
  var a = data['graph'];
...
//CHANGE TOP GRAPH
el('top_graph').onclick = () => moveCamera(pos.graph);
el('top_graph').onchange = () => {
  moveCamera(pos.graph);
  changeGraph(el('top_graph').value, 'left');
};
